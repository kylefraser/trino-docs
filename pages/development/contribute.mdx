...
2. Start a discussion by creating a GitHub
   ...
   3. Will the change conflict with another change in progress? If so, work with
...
3. Implement the change
   ...
   1. Create or update
      [your own fork of the repository](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-forks/about-forks)
...
4. Create a GitHub [pull request]({{site.github_repo_url}}/pulls) (PR).
   ...
   2. Refer to the
      [GitHub documentation for more details about collaborating with PRs](https://docs.github.com/en/pull-requests).
   3. Make sure the pull request passes the tests in CI.
   4. If known, request a review from an expert in the area changed. If unknown,
      ask for help on [Slack](/slack.html).
...
5. Review is performed by one or more reviewers.
   ...
   1. This normally happens within a few days, but may take longer if the change
      is large, complex, or if a critical reviewer is unavailable. (feel free to
      ping the reviewer or
      [DevRel team](https://github.com/orgs/trinodb/teams/devrel/members) on the
      pull request).
...
8. Maintainers organize a release to ship the next Trino version with your
   improvements.
